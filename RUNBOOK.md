# Griot Moon — Operational Runbook
...
4. [IONOS (DNS for griotmoon.com)](#3-ionos-dns-for-griotmooncom)
...
|---|---|---|---|---|
| GitHub | github.com | `PawaSeyni` org | Free | repo: `PawaSeyni/griotmoon` |
...
| Pinterest | pinterest.com | Pawa Seyni's account (verified) | Free | domain verify code `54badc00b81b728d0e9295635e297353` |
...
```bash
cd "/Users/papasnguer/Desktop/Organized/17_Completed_Books_Archive/Completed Books/Pawa Seyni/griotmoon"
...
**Purpose:** Builds the Vite project on each push, serves the static site at griotmoon.com, captures Contact form submissions, runs Lighthouse audits.

**Dashboard:** https://app.netlify.com/projects/griotmoon
**Project name:** `griotmoon`
...
- Primary: `griotmoon.com` (HTTPS via Let's Encrypt, auto-renews ~60 days before expiry)
- Alias: `www.griotmoon.com` (CNAME → netlify.app)
- Legacy: `griotmoon.netlify.app` (301-redirects to apex via `netlify.toml` rule)
...
| **Custom domain not resolving** | DNS resolution via `dns.google/resolve?name=griotmoon.com&type=A` (must return `75.2.60.5`) | IONOS DNS settings (see section 3) |

### Smoke tests

After any deploy, navigate to:
- `https://griotmoon.com/` — home loads
- `https://griotmoon.com/books` — book grid renders, images load
- `https://griotmoon.com/bilingual-starter-kit.pdf` — PDF serves
- `https://griotmoon.netlify.app/` — should 301-redirect to apex

---

## 3. IONOS (DNS for griotmoon.com)

**Purpose:** Domain registrar and DNS provider. Domain points to Netlify.

**Dashboard:** Log into IONOS customer area → Domains & SSL → click `griotmoon.com` → DNS section.
...
| CNAME | `www` | `griotmoon.netlify.app` | www → Netlify |
...
| DNS changes not propagating | Wait up to TTL (3600s = 1 hour). Verify via https://dnschecker.org/#A/griotmoon.com |
...
| Monthly | Export subscriber list (Subscribers → Export) as backup CSV. Save to `/Pawa Seyni/MailerLite_Backups/YYYY-MM.csv` |
...
**Purpose:** Brand presence for Griot Moon. Linked from site Footer + Contact.

**URL:** https://facebook.com/griotmoon
**Vanity URL secured:** Yes (`griotmoon`)
**Admin:** Pawa Seyni (personal profile `61590158550516`)
...
**Author URL:** https://www.amazon.com/author/pawaseyni
**Dashboard:** https://kdp.amazon.com (Pawa Seyni's account)
...
**Account:** Pawa Seyni's Pinterest
**Site verification:** Added via `<meta name="p:domain_verify" content="54badc00b81b728d0e9295635e297353"/>` in `index.html`

This site is verified with Pinterest but no Pin-related code is on the site yet. Pinterest pins exist in `/Pawa Seyni/Online Strategy/` and `/Pawa Seyni/Social_Media_Complete_Package/` — to be scheduled with a tool like Tailwind or posted manually.
...
1. Open https://griotmoon.com/ in incognito → does it load?
...
5. If "Published" but site still down → check `dns.google/resolve?name=griotmoon.com&type=A` returns `75.2.60.5`
...
2. Commit + push
3. Verify URL: `https://griotmoon.com/your-new-magnet.pdf` returns 200, application/pdf
...
| Credential | Storage |
|---|---|
| GitHub password | Pawa Seyni's password manager (1Password, Bitwarden, etc.) |
| Netlify access | Google SSO (galloeva2612@gmail.com) |
| MailerLite | Browser session (galloeva2612@gmail.com) |
| IONOS | Pawa Seyni's password manager |
...
- Export MailerLite subscriber CSV → archive to `/Pawa Seyni/MailerLite_Backups/YYYY-MM.csv`
