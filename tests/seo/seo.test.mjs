// Per-route SEO assertions (parity plan P3-2), ported from Story Time with Eva. Runs
// against the PRERENDERED dist/, what crawlers actually receive, so it must run AFTER
// `npm run build` (npm run test:seo, part of `npm run verify`). It complements
// scripts/check-seo-output.mjs (canonical, trailing slash, dead links) with titles,
// descriptions, Open Graph, locale, <html lang> and JSON-LD.
//
...
function routesFromSitemap() {
  // The sitemap the build generated and ships, not a committed copy.
  const xml = readFileSync(path.join(ROOT, 'dist', 'sitemap.xml'), 'utf8');
...
// Pages whose component renders <JsonLd>: Home, Books, BookDetail, FAQ, About, ResourceArticle.
const JSONLD_PATHS = (p) => p === '/' || p === '/books' || p.startsWith('/books/') || p === '/faq' || p === '/about' || /^\/resources\/[^/]+$/.test(p);
