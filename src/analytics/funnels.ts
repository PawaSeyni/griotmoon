...
export const FUNNELS: FunnelDefinition[] = [
  ...
  { id: 'landing-entrances', title: 'Landing page → form start → lead',
    steps: [{ event: 'Landing View' }, { event: 'Form Start', where: { placement: 'landing' } }, { event: 'Lead Created', where: { placement: 'landing' } }],
    dimensions: ['lead_magnet', 'language'], measures: 'outcome',
    notes: 'One offer per page, so this is the per-offer conversion a pin or an ad is judged on.' },
