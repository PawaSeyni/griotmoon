// Server-side newsletter subscribe (parity plan P2-1), ported from Story Time with Eva.
//
// The form used to POST from the browser straight to MailerLite's JSONP embedded-form
// endpoint with mode:'no-cors', so the page could not read the answer and showed
// "success" whatever happened. On Eva that exact setup dropped every email for days
// when the endpoint began returning 503. This function calls MailerLite's official
// API server-side with the account token (never exposed to the browser) and returns
// a real success/error the form can act on, which is what makes `Lead Created` mean
// a subscriber MailerLite actually created.
//
...
export async function handler(event) {
  ...
  if (origin) {
    ...
    // (`<branch>--griotmoon.netlify.app`), and allow localhost ONLY under
    // `netlify dev` (CONTEXT=dev), never in production.
    const isOwnHost =
      host === 'griotmoon.com' ||
      host === 'www.griotmoon.com' ||
      /^([a-z0-9-]+--)?griotmoon\.netlify\.app$/.test(host);
    const isDevHost =
  ...
  // The welcome automation is triggered by joining `griotmoon-signups`, so
